...
- `overlay.rs` / `chip_handler.rs`: 結果表示と、OCR・翻訳・解説エンジン／プロファイル別チップからの再実行。通常翻訳以外の結果見出しを保持し、その間は翻訳エンジンを未選択表示にする。`WS_EX_NOACTIVATE`を維持しつつ、クリック時は`app_state`へ保存した表示起点ウィンドウを前面化する。【コピー中の内容】はリッチ形式をMarkdownとして、プレーン形式を空白・改行を削らず取り込む。
